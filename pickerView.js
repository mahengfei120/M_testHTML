...
(function() {
    ...
    PickerView.prototype = {
        ...
        init: function() {
            ...
            this.elem_wrap = div;
            this.padding = document.documentElement.clientHeight * 0.21 - 61;
            ...
            this.elem_rightBtn.addEventListener("touchend", function(e) {
                ...
                let cityName = selectArr[1]
                let areaName = selectArr[2]
                let cityCode = ''
                let chooseAreaCode = ''
                if (cityName && cityName == '济源市') {
                  cityCode = '419001'
                  chooseAreaCode = '419001'
                } else {
                  for (let i = 0; i < cityData.length; i++) {
                    if (cityData[i].AreaName == cityName) {
                      cityCode = cityData[i].AreaCode
                      for (let j = 0; j < cityData[i].ChildAreaCode.length; j++) {
                        const item = cityData[i].ChildAreaCode[j]
                        if (item.AreaName == areaName) {
                          chooseAreaCode = item.AreaCode
                        }
                      }
                    }
                  }
                }
                console.log('当前选择的是：河南省' + cityName + areaName + '-' + chooseAreaCode)
                const chooseData = {
                  provinceName: '河南省',
                  cityName,
                  cityCode,
                  areaName: areaName || '',
                  AreaCode: chooseAreaCode || ''
                }
                _this.Opt.rightFn(chooseData);
                _this.closeComponent();
                // 绑定元素
        ...
        changeNext: function(index) {
            ...
            for (var i = 0; i < this.selectArr.length; i++) {
                ...
                if (i > index) {
                    ...
                    var field = elem_items.children[this.selectArr[i]] ? elem_items.children[this.selectArr[i]].innerText : '';
                    data = data[field];
                }
            }
        },
        ...
        initData: function () {
          const cityData = this.Opt.data
          const changeList = {
            '河南': {}
          }
          for (let i = 0; i < cityData.length; i++) {
            changeList['河南'][cityData[i].AreaName] = []
            if (cityData[i].ChildAreaCode) {
              for (let j = 0; j < cityData[i].ChildAreaCode.length; j++) {
                changeList['河南'][cityData[i].AreaName].push(cityData[i].ChildAreaCode[j].AreaName)
              }
            }
          }
          this.Opt.data = changeList
        }
    }
